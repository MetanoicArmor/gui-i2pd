...
*Окно настроек с интерактивными портами HTTP/SOCKS5, управлением пропускной способностью и автозапуском через LaunchAgent*

![I2P GUI Расширенные настройки](screenshot4.png)
...
- **🎨 Интерфейс**: управление темами оформления
- **📊 Динамические значения**: загрузка настроек напрямую from конфиг файлов
...
### 🏆 **v2.58.0 (Current)** - Стабильная версия с полным функционалом
...
```bash
Sources/i2pd-gui/
└── AppCore.swift        # Все исходники в одном файле (3,400+ строк)
                        # - ContentView: основной интерфейс с адаптивностью
                        # - SettingsView: интерактивные настройки портов и скорости
                        # - I2pdManager: управление daemon и LaunchAgent
                        # - Система логирования с темами оформления
                        # - TrayManager: стабильный системный трей
...
build-app-simple.sh      # Скрипт сборки .app пакета с код-сигнатурой
