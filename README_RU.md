...
**Сразу после установки или обновления** снимите карантин загрузки (в Homebrew **4.7+** убрали флаг **`--no-quarantine`**, замены в `brew` нет):
...
На свежих версиях macOS Gatekeeper может сразу показать **«приложение повреждено»** и **не** предложить **«Всё равно открыть»** в настройках — это другой сценарий, чем «неизвестный разработчик». Пока приложение без **Developer ID** и **нотаризации**, рабочий обход — **`xattr`** после `brew install` / `brew upgrade`.
