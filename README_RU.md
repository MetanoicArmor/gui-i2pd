...
[![Download](https://img.shields.io/badge/📥%20Скачать-I2P%20Daemon%20GUI.app-brightgreen.svg)](https://github.com/MetanoicArmor/gui-i2pd/releases/download/v2.61.0/I2P-Daemon-GUI-2.61.0.zip)
...
В cask добавлен шаг `postflight`, который автоматически выполняет `xattr -dr com.apple.quarantine` на установленный `.app` — в большинстве случаев приложение открывается без дополнительных действий.

Если macOS всё равно пишет, что приложение **«повреждено»** (Gatekeeper / карантин для не нотаризованных загрузок), снимите карантин вручную:

```bash
xattr -dr com.apple.quarantine "/Applications/I2P Daemon GUI.app"
```

На свежих версиях macOS кнопку **«Всё равно открыть»** в настройках для этого случая могут не показать — она относится к сценарию «неизвестный разработчик», а не «повреждено». Долгосрочное решение — подпись **Developer ID** и **нотаризация** в Apple.
...
- **Daemon**: i2pd 2.61 (встроенный бинарник)
...
I2P Daemon GUI.app/
├── Contents/
│   ├── Info.plist              # Метаданные (версия 2.61)
│   ├── MacOS/
...
### 🏆 **v2.61.0 (текущая)** — i2pd 2.61.0 (arm64 + Intel)

### **v2.60.0** — обновление UI под macOS 26, i2pd 2.60, ZIP релиза и cask в Homebrew (arm64)
...
Info.plist              # Метаданные приложения (версия 2.61)
```
...
### 🚀 Прямое скачивание:
[![Download I2P Daemon GUI.app](https://img.shields.io/badge/📦%20Скачать-I2P%20Daemon%20GUI.app-ff6b6b.svg?style=for-the-badge)](https://github.com/MetanoicArmor/gui-i2pd/releases/download/v2.61.0/I2P-Daemon-GUI-2.61.0.zip)
